...
const api = global.bedrock = new Helper();
module.exports = api;

// expose mocha to helper, overwrite `mocha.run` to allow unit tests
// to be executed client side and results reported back
const realPathMocha = fs.realpathSync(require.resolve('mocha'));
const Mocha = require(realPathMocha);
const originalMochaRun = Mocha.prototype.run;
Mocha.prototype.run = function() {
  api.mocha = this;
  api.suite = null;

  // update runner to integrate browser-mocha results
  const runner = api.runner = originalMochaRun.apply(this, arguments);
  ...
  function processResult(suite, node) {
    ...
    if(node.type === 'suite') {
      const newSuite = new Mocha.Suite(node.title);
...
Helper.prototype.run = function(fn) {
  fn = fn.toString();
  const isAsync = (fn.split('\n')[0].indexOf(',') !== -1);
  const execute = isAsync ? browser.executeAsyncScript : browser.executeScript;
  return execute(
    "const $injector = angular.element('" + browser.rootEl + "')" +
      ".data('$injector');" +
...
/**
 * Wait for the most recent popup to close
 *
 * Note: this method is not robust enough to handle waiting
 * for a popup in the middle of the stack; only the most recent
 * popup can be waited on.
 *
 * @param  index the index of the popup. E.g., 0=browser, 1=FBpopup
 * @return webdriver.promise.Promise
 */
Helper.prototype.waitForPopupClose = function(index) {
  // wait for there to be less handles open than the index of the popup
  browser.wait(function() {
    return browser.getAllWindowHandles().then(function(handles) {
      /**
       * Assume that handles.length >= 1 and index >=0.
...
Helper.prototype.unit = function(title, fn, options) {

  // FIXME: disabling unit tests
  return;

  const self = this;
  options = options || {};
  const suite = Mocha.Suite.create(self.mocha.suite, title);
  ...
  suite.testInBrowser = function() {
    ...
    return self.get('/bedrock-protractor-unit', {
      ...
      return browser.executeAsyncScript(
        "const callback = arguments[arguments.length - 1];" +
        // TODO: bug/misunderstanding causes `bedrock-protractor-unit` to
        // executed twice when imported again... so must call global until
        // issue is properly resolved
        // "SystemJS.import('bedrock-protractor-unit').then(unit => {" +
        // "unit.default('" + browser.rootEl + "', " + fn + ", callback)});");
        "_bedrock_protractor_unit('" +
          browser.rootEl + "', " + fn + ", callback);");
    }).catch(function(err) {
      if(!options.debug) {
        throw err;
      }
      logged = true;
      // log any output from the browser
      return _logBrowserConsole().then(function() {
        throw err;
      });
    }).then(function(result) {
      if(logged || !options.debug) {
        return result.root;
      }
      // log any output from the browser
      return _logBrowserConsole().then(function() {
  ...
  if(suite.tests.length === 0) {
    added = true;
    const test = new Mocha.Test('dummy', function() {});
    test.parent = suite;
    test.ctx = suite.ctx;
...
Helper.prototype.waitForUnit = function() {
  ...
  return browser.driver.wait(() => {
    return browser.driver.findElements(by.css('html[bedrock-protractor-unit]'))
...
Helper.prototype.waitForElement = function(el) {
  ...
  return browser.wait(function() {
    return el.isPresent();
  }, self.MAX_TIMEOUT, 'should wait for element to be present');
};
...
Helper.prototype.waitForUrl = function(url) {
  ...
  if(typeof url === 'function') {
    filter = function() {return browser.driver.getCurrentUrl().then(url);};
  } else {
  ...
  return browser.wait(filter, self.MAX_TIMEOUT, 'should wait for url: ' + url);
};
...
Helper.prototype.randomString = function(length) {
  const idx_A = 'A'.charCodeAt(0);
  const idx_a = 'a'.charCodeAt(0) - 26;
  length = length || 10;
  let rval = '';
  while(rval.length < length) {
    let code = Math.floor(Math.random() * 52);
...
Helper.prototype.formatDate = function(date, format) {
  return this.run(['function($injector, callback) {',
    'const filter = $injector.get("dateFilter");',
